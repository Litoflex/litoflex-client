	...
	return (
		...
		>
			<div className="flex flex-row justify-center items-center gap-1">
				...
				/>
				<div>
					<p className="c_name font-bold text-3xl text-[#4A3E3E]">
						{item.name}
					</p>
					<p className="c_size text-xl text-[#4A3E3E]">
						Размер{" "}
						<span className="font-bold">{item.size}</span>
					</p>
					<p className="c_width text-xl text-[#4A3E3E]">
						Толщина{" "}
						<span className="font-bold">{item.width}</span>
					</p>
				</div>
			</div>
			...
			<div className="flex flex-col items-end justify-between h-30 p-2">
				...
				<p className="c_price font-bold text-3xl text-[#4A3E3E]">
					{item.price}
				</p>
			</div>
		</motion.div>
...
export default function CategoryPageClient({
	...
	catalogueData: Array<Catalogue>;
}) {
	const params = useParams();
	const cat: Catalogue | undefined = catalogueData.filter(
		(p) => p.key === params.id
	)[0];
